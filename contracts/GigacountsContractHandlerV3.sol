...
contract GigacountsContractHandlerV3 {
    ...
    function getFunds(uint256 _contractId, address _tokenAddress) external view returns (uint256) {
        require(_tokenAddress != address(0), "Invalid address");
        return contractData[_contractId].contractFunds[_tokenAddress].totalFunds;
    }

    function sendFunds(uint256 _contractId, address _tokenAddress, uint256 _amount) external returns (bool) {
        ...
        require(_tokenAddress != address(0), "Invalid address");
        contractData[_contractId].contractFunds[_tokenAddress].tokenAddress = address(_tokenAddress);
    ...
    function makePayment(uint256 _contractId) external onlyOwner returns (bool) {
        // TODO
    }
