...
contract GigacountsContractHandlerV4 {
    ...
    function fundContract(uint256 _contractId, address _tokenAddress, uint256 _amount) internal tokenSupported(_tokenAddress) returns (bool) {
        ...
        /*
        contractData[_contractId].contractFunds[_tokenAddress].tokenAddress = _tokenAddress;
        contractData[_contractId].contractFunds[_tokenAddress].totalFunds += _amount;
        contractData[_contractId].contractFunds[_tokenAddress].receivedFunds += _amount;
        */
        return true;
    }

    function makePayment(uint256 _contractId, address _tokenAddress, uint256 _amount, address _walletAddress) external onlyOwner tokenSupported(_tokenAddress) returns (bool) {
        require(contractData[_contractId].contractId != 0, "Contract with this contractId does not exist");
        require(_tokenAddress != address(0), "Invalid token address");
        require(_walletAddress != address(0), "Invalid wallet address");
        ...
        // contractData[_contractId].contractFunds[_tokenAddress].totalFunds -= _amount;
        // contractData[_contractId].contractFunds[_tokenAddress].payoutFunds += _amount;
        return true;
    }

    function cashback(uint256 _contractId, address _tokenAddress) external onlyOwner tokenSupported(_tokenAddress) returns (bool) {
        require(contractData[_contractId].contractId != 0, "Contract with this contractId does not exist");
        require(_tokenAddress != address(0), "Invalid token address");

        uint256 payoutFunds = contractData[_contractId].contractFunds[_tokenAddress].payoutFunds;
        uint256 totalFunds = contractData[_contractId].contractFunds[_tokenAddress].totalFunds;
        require(totalFunds > payoutFunds, "Insufficient balance for proceed with cashback");

        uint256 cashbackToTransfer = totalFunds - payoutFunds;
    ...
    function withdrawFunds(uint256 _contractId, address payable _recipient, address _tokenAddress) external onlyOwner tokenSupported(_tokenAddress) returns (bool) {
        require(contractData[_contractId].contractId != 0, "Contract with this contractId does not exist");
